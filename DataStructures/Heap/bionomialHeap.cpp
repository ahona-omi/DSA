...
public:
    ...
    node *Search(node *H, int k){
        ...
        if (x->child != NULL && p == NULL)    p = Search(x->child, k);
        if (x->sibling != NULL && p == NULL)    p = Search(x->sibling, k);
        return p;
    }
    void Decrease_key(node *H, int i, int k){
        int temp;
        node *p;
        node *y;
        node *z;
    ...
    void Delete(node *H, int k){
        ...
        np = Extract_Min(H);
        if (np != NULL)    cout << "Node Deleted Successfully" << endl;
    }
};
int main(){
    int n, m, l, i;
    ...
    while (1){
        ...
        case 2:
            p = bh.Extract_Min(H);
            if (p != NULL)    cout << "The node with minimum key: " << p->n << endl;
